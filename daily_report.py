...
from trade_history import INITIAL_CAPITAL_EUR, STATUS_OPEN, STATUS_SL, STATUS_TP1, TradeHistory
...
def build_daily_report(trades, now=None):
    ...
    return (
        ...
        f"Capitale iniziale: {INITIAL_CAPITAL_EUR:.2f} €\n"
        f"Capitale composto attuale: {total_stats['compound_capital']:.2f} €\n"
        f"Rendimento composto: {total_stats['compound_return']:+.2f}%\n"
...
def _stats(trades):
    ...
    compound_capital = round(INITIAL_CAPITAL_EUR + net_profit, 2)
    compound_return = (
        (compound_capital - INITIAL_CAPITAL_EUR) / INITIAL_CAPITAL_EUR * 100
        if INITIAL_CAPITAL_EUR
        else 0.0
    )
...
def _format_recent_trades(trades):
    ...
    for trade in sorted(trades, key=lambda item: item.get("created_at", "")):
        lines.append(
            ...
            f"Investimento {float(trade.get('investment_eur', INITIAL_CAPITAL_EUR)):.2f} €\n"
